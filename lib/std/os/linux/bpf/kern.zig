...
pub fn BigEndian(comptime T: type) type {
    switch (T) {
        u16, u32, u64 => {},
        else => @compileError("underlying type must be u16, u32, or u64"),
    }
...
pub const BpfSockAddr = packed struct {
    ...
    /// (*addr*\ **->sa_family**) must be **AF_INET** (or **AF_INET6**). It's
    /// advised to pass zero port (**sin_port** or **sin6_port**) which triggers
    ...
    pub const bind: fn (ctx: *BpfSockAddr, addr: *SockAddr, addr_len: i32) !void = @compileError("TODO");
};
...
pub const map_lookup_elem: fn (comptime Key: type, comptime Value: type, map: *const MapDef, key: *const Key) ?*Value = @compileError("TODO");
...
pub const map_update_elem: fn (comptime Key: type, comptime Value: type, map: *const MapDef, key: *const Key, value: *const Value, flags: u64) !void = @compileError("TODO");

/// Delete entry with *key* from *map*.
pub const map_delete_elem: fn (comptime Key: type, map: *const MapDef, key: *const Key) !void = @compileError("TODO");

/// For tracing programs, safely attempt to read *size* bytes from kernel space
/// address *unsafe_ptr* and store the data in *dst*.
///
/// Generally, use **bpf_probe_read_user**\ () or **bpf_probe_read_kernel**\ ()
/// instead.
pub const probe_read: fn (dst: []u8, src: []u8) !void = @compileError("TODO");

/// Return the time elapsed since system boot, in nanoseconds.  Does not include
/// time the system was suspended.  See: **clock_gettime**\
/// (**CLOCK_MONOTONIC**)
...
/// *\/sys/kernel/debug/tracing/trace* from DebugFS, if available. It can take
/// up to three additional **u64** arguments (as an eBPF helpers, the total
...
/// *\/sys/kernel/debug/tracing/trace_pipe* to avoid this.  The format of the
/// trace is customizable, and the exact output one will get depends on the
/// options set in *\/sys/kernel/debug/tracing/trace_options* (see also the
/// *README* file under the same directory). However, it usually defaults to
/// something like:
///
/// ::
///
...
/// 	  are enabled, scheduling options, whether hard/softirqs are running,
/// 	  level of preempt_disabled respectively). **N** means that
...
/// than for printk(). They are **%d**, **%i**, **%u**, **%x**, **%ld**,
/// **%li**, **%lu**, **%lx**, **%lld**, **%lli**, **%llu**, **%llx**, **%p**,
/// **%s**. No modifier (size of field, padding with zeroes, etc.) is available,
/// and the helper will return **-EINVAL** (but print nothing) if it encounters
/// an unknown specifier.
///
/// Also, note that **bpf_trace_printk**\ () is slow, and should only be used
/// for debugging purposes. For this reason, a notice block (spanning several
...
/// precisely, when **trace_printk**\ () buffers are allocated). For passing
/// values to user space, perf events should be preferred.
///
/// Returns the number of bytes written to the buffer
pub const trace_printk: fn (fmt: []const u8, args: anytype) !usize = @compileError("TODO");
...
/// bytes) and **BPF_F_INVALIDATE_HASH** (set *skb*\ **->hash**, *skb*\
...
pub const skb_store_bytes: fn (skb: *SkBuff, offset: u32, from: []const u8, flags: u64) !void = @compileError("TODO");
...
/// header field that was modified (*from*), the new value of this field (*to*),
/// and the number of bytes (2 or 4) for this field, stored in *size*.
/// Alternatively, it is possible to store the difference between the previous
/// and the new values of the header field in *to*, by setting *from* and *size*
/// to 0. For both methods, *offset* indicates the location of the IP checksum
/// within the packet.
///
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const l3_csum_replace: fn (skb: *SkBuff, offset: u32, from: u64, to: u64, size: u64) !void = @compileError("TODO");
...
/// former value of the header field that was modified (*from*), the new value
/// of this field (*to*), and the number of bytes (2 or 4) for this field,
/// stored on the lowest four bits of *flags*. Alternatively, it is possible to
/// store the difference between the previous and the new values of the header
/// field in *to*, by setting *from* and the four lowest bits of *flags* to 0.
/// For both methods, *offset* indicates the location of the IP checksum within
/// the packet. In addition to the size of the field, *flags* can be added
/// (bitwise OR) actual flags. With **BPF_F_MARK_MANGLED_0**, a null checksum is
/// left untouched (unless **BPF_F_MARK_ENFORCE** is added as well), and for
/// updates resulting in a null checksum the value is set to **CSUM_MANGLED_0**
/// instead. Flag **BPF_F_PSEUDO_HDR** indicates the checksum is to be computed
/// against a pseudo-header.
///
/// This helper works in combination with **bpf_csum_diff**\ (), which does not
/// update the checksum in-place, but offers more flexibility and can handle
/// sizes larger than 2 or 4 for the checksum to update.
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const l4_csum_replace: fn (skb: *SkBuff, offset: u32, from: u64, to: u64, flags: u64) !void = @compileError("TODO");
...
/// the kernel by the macro **MAX_TAIL_CALL_CNT** (not accessible to user
/// space), which is currently set to 32.
pub const tail_call: fn (ctx: anytype, prog_array_map: *const MapDef, index: u32) !void = @compileError("TODO");
...
/// executed out of the eBPF program. Conversely, **bpf_redirect**\ () is more
/// efficient, but it is handled through an action code where the redirection
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const clone_redirect: fn (skb: *SkBuff, ifindex: u32, flags: u64) !void = @compileError("TODO");
...
pub const get_current_comm: fn (buf: []u8) !void = @compileError("TODO");
...
pub const get_cgroup_classid: fn (skb: *SkBuff) u32 = @compileError("TODO");

/// Push a *vlan_tci* (VLAN tag control information) of protocol *vlan_proto* to
/// the packet associated to *skb*, then update the checksum. Note that if
/// *vlan_proto* is different from **ETH_P_8021Q** and **ETH_P_8021AD**, it is
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const skb_vlan_pop: fn (skb: *SkBuff) !void = @compileError("TODO");

/// Get tunnel metadata. This helper takes a pointer *key* to an empty **struct
/// bpf_tunnel_key** of **size**, that will be filled with tunnel metadata for
/// the packet associated to *skb*.  The *flags* can be set to
/// **BPF_F_TUNINFO_IPV6**, which indicates that the tunnel is based on IPv6
...
/// exposes the *key*\ **->tunnel_id**, which is generally mapped to a VNI
...
/// IP in IP (IPIP).
pub const skb_get_tunnel_key: fn (skb: *SkBuff, key: *TunnelKey, size: u32, flags: u64) !void = @compileError("TODO");
...
pub const skb_set_tunnel_key: fn (skb: *SkBuff, key: *TunnelKey, size: u32, flags: u64) !void = @compileError("TODO");
...
pub const perf_event_read: fn (map: *const MapDef, flags: u64) u64 = @compileError("TODO");

/// Redirect the packet to another net device of index *ifindex*.  This helper
/// is somewhat similar to **bpf_clone_redirect**\ (), except that the packet is
/// not cloned, which provides increased performance.
///
...
/// **bpf_redirect_map**\ (), which uses a BPF map to store the redirect target
/// instead of providing it directly to the helper.
///
/// Returns for XDP, the helper returns **XDP_REDIRECT** on success or
/// **XDP_ABORTED** on error. For other program types, the values are
/// **TC_ACT_REDIRECT** on success or **TC_ACT_SHOT** on error.
pub const redirect: fn (ifindex: u32, flags: u64) !void = @compileError("TODO");

/// Retrieve the realm or the route, that is to say the **tclassid** field of
/// the destination for the *skb*. The identifier retrieved is a user-provided
/// tag, similar to the one used with the net_cls cgroup (see description for
/// **bpf_get_cgroup_classid**\ () helper), but here this tag is held by a route
/// (a destination entry), not by a task.
///
...
/// artificially held via **netif_keep_dst**\ () for a classful qdisc until the
/// *skb* is freed.
///
...
/// none was found.
pub const get_route_realm: fn (skb: *SkBuff) u32 = @compileError("TODO");
...
pub const perf_event_output: fn (ctx: anytype, map: *const MapDef, flags: u64, data: []u8) !void = @compileError("TODO");

/// This helper was provided as an easy way to load data from a packet. It can
/// be used to load *len* bytes from *offset* from the packet associated to
/// *skb*, into the buffer pointed by *to*.
///
/// Since Linux 4.7, usage of this helper has mostly been replaced by "direct
/// packet access", enabling packet data to be manipulated with *skb*\
/// **->data** and *skb*\ **->data_end** pointing respectively to the first byte
...
pub const skb_load_bytes: fn (skb: anytype, offset: u32, to: []u8) !void = @compileError("TODO");

/// Walk a user or a kernel stack and return its id. To achieve this, the helper
/// needs *ctx*, which is a pointer to the context on which the tracing program
...
/// to 255), masked with **BPF_F_SKIP_FIELD_MASK**. The next bits can be used to
...
/// (), which can be used with unrolled loops but is not efficient and consumes
/// a lot of eBPF instructions.  Instead, **bpf_get_stackid**\ () can collect up
/// to **PERF_MAX_STACK_DEPTH** both kernel and user frames. Note that this
/// limit can be controlled with the **sysctl** program, and that it should be
...
///
/// Returns the positive or null stack id on success, or a negative error in
/// case of failure.
...
/// added to the value (this can be cascaded, the seed may come from a previous
/// call to the helper).
///
/// This is flexible enough to be used in several ways:
///
/// * With *from_size* == 0, *to_size* > 0 and *seed* set to checksum, it can be
///   used when pushing new data.
/// * With *from_size* > 0, *to_size* == 0 and *seed* set to checksum, it can be
///   used when removing data from a packet.
...
/// **bpf_l4_csum_replace**\ (), to which one can feed in the difference
/// computed with **bpf_csum_diff**\ ().
///
...
/// of **bpf_skb_get_tunnel_key**\ () for more details). A particular example
/// where this can be used is in combination with the Geneve encapsulation
/// protocol, where it allows for pushing (with **bpf_skb_get_tunnel_opt**\ ()
/// helper) and retrieving arbitrary TLVs (Type-Length-Value headers) from the
/// eBPF program. This allows for full customization of these headers.
///
/// Returns the size of the option data retrieved.
pub const skb_get_tunnel_opt: fn (skb: *SkBuff, opt: []u8) usize = @compileError("TODO");
...
/// additional information.
pub const skb_set_tunnel_opt: fn (skb: *SkBuff, opt: []u8) !void = @compileError("TODO");

/// Change the protocol of the *skb* to *proto*. Currently supported are
...
/// **bpf_l3_csum_replace**\ () and **bpf_l4_csum_replace**\ (). The main case
/// for this helper is to perform NAT64 operations out of an eBPF program.
///
...
/// have a write access to *skb*\ **->pkt_type** beside this helper. Using a
...
pub const skb_change_type: fn (skb: *SkBuff, type: u32) !void = @compileError("TODO");
...
///
/// Returns the return value depends on the result of the test, and can be:
///     * 0, if the *skb* failed the cgroup2 descendant test.
/// 	* 1, if the *skb* succeeded the cgroup2 descendant test.
/// 	* A negative error code, if an error occurred.
pub const skb_under_cgroup: fn (skb: *SkBuff, map: *const MapDef, index: u32) !void = @compileError("TODO");

/// Retrieve the hash of the packet, *skb*\ **->hash**. If it is not set, in
/// particular if the hash was cleared due to mangling, recompute this hash.
...
/// **bpf_skb_change_proto**\ (), or calling **bpf_skb_store_bytes**\ () with
...
/// ().
pub const get_hash_recalc: fn (skb: *SkBuff) u32 = @compileError("TODO");

/// Return a pointer to the current task struct.
pub const get_current_task = helpers.get_current_task;

/// Attempt in a safe way to write *len* bytes from the buffer *src* to *dst* in
/// memory. It only works for threads that are in user context, and *dst* must
/// be a valid user space address.
///
...
pub const probe_write_user: fn (dst: *u8, src: []u8) !void = @compileError("TODO");
...
/// 	* A negative error code, if an error occurred.
pub const current_task_under_cgroup: fn (map: *const MapDef, index: u32) !bool = @compileError("TODO");

/// Resize (trim or grow) the packet associated to *skb* to the new *len*. The
...
/// **bpf_l3_csum_replace**\ () and others. This helper is a slow path utility
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const skb_change_tail: fn (skb: *SkBuff, len: u32, flags: u64) !void = @compileError("TODO");

/// Pull in non-linear data in case the *skb* is non-linear and not all of *len*
/// are part of the linear section. Make *len* bytes from *skb* readable and
/// writable. If a zero value is passed for *len*, then the whole length of the
/// *skb* is pulled.
///
...
/// boundaries (test on *skb*\ **->data_end**) is susceptible to fail if offsets
/// are invalid, or if the requested data is in non-linear parts of the *skb*.
/// On failure the program can just bail out, or in the case of a non-linear
/// buffer, use a helper to make the data available. The **bpf_skb_load_bytes**\
/// () helper is a first solution to access the data. Another one consists in
/// using **bpf_skb_pull_data** to pull in once the non-linear parts, then
/// retesting and eventually access the data.
///
...
/// calling **bpf_skb_pull_data()** to effectively unclone the *skb* from the
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const skb_pull_data: fn (skb: *SkBuff, len: u32) !void = @compileError("TODO");

/// Add the checksum *csum* into *skb*\ **->csum** in case the driver has
...
/// **bpf_csum_diff**\ (), in particular when the checksum needs to be updated
...
pub const set_hash_invalid: fn (skb: *SkBuff) void = @compileError("TODO");
...
/// **socket(7)**), but the helper is also available to other eBPF program
/// types, similarly to **bpf_get_smp_processor_id**\ ().
pub const get_numa_node_id = helpers.get_numa_node_id;

/// Grows headroom of packet associated to *skb* and adjusts the offset of the
/// MAC header accordingly, adding *len* bytes of space. It automatically
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const skb_change_head: fn (skb: *SkBuff, len: u32, flags: u64) !void = @compileError("TODO");
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const xdp_adjust_head: fn (xdp_md: *XdpBuff, delta: i32) !void = @compileError("TODO");

/// Copy a NUL terminated string from an unsafe kernel address *unsafe_ptr* to
/// *dst*. See **bpf_probe_read_kernel_str**\ () for more details.
///
/// Generally, use **bpf_probe_read_user_str**\ () or
/// **bpf_probe_read_kernel_str**\ () instead.
///
/// Returns the length of the string including the trailing null character
pub const probe_read_str: fn (dst: []u8, unsafe: [*:0]const u8) !usize = @compileError("TODO");

/// If the **struct sk_buff** pointed by *ctx* has a known socket, retrieve the
...
///
/// ctx is either of type *SkBuff, *BpfSockAddr, *SockOps
///
/// Returns A 8-byte long non-decreasing number on success, or null if the
/// socket field is missing inside *ctx*.
pub const get_socket_cookie: fn (ctx: anytype) ?u64 = @compileError("TODO");

/// The owner UID of the socket associated to *skb*. If the socket is **NULL**,
...
pub const get_socket_uid: fn (skb: *SkBuff) u32 = @compileError("TODO");

/// Set the full hash for *skb* (set the field *skb*\ **->hash**) to value
/// *hash*.
pub const set_hash: fn (skb: *SkBuff, hash: u32) !void = @compileError("TODO");

/// Emulate a call to **setsockopt()** on the socket associated to *bpf_socket*,
/// which must be a full socket. The *level* at which the option resides and the
/// name *optname* of the option must be specified, see **setsockopt(2)** for
/// more information.  The option value of length *optlen* is pointed by
/// *optval*.
///
...
pub const setsockopt: fn (socket: anytype, level: i32, opname: i32, opt: []u8) !void = @compileError("TODO");
...
///
/// * **BPF_F_ADJ_ROOM_NO_CSUM_RESET**: Do not reset offloaded checksum data of
///   the skb to CHECKSUM_NONE.
///
...
///
///	* **BPF_F_ADJ_ROOM_ENCAP_L3_IPV4**, **BPF_F_ADJ_ROOM_ENCAP_L3_IPV6**: Any
///	  new space is reserved to hold a tunnel header.  Configure skb offsets and
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const skb_adjust_room: fn (skb: *SkBuff, len_diff: i32, mode: u32, flags: u64) !void = @compileError("TODO");
...
/// codes up to **XDP_TX**, as chosen by the caller. Any higher bits in the
/// *flags* argument must be unset.
///
/// See also **bpf_redirect**\ (), which only supports redirecting to an
/// ifindex, but doesn't require a map to do so.
///
/// Returns **XDP_REDIRECT** on success, or the value of the two lower bits of
/// the *flags* argument on error.
pub const redirect_map: fn (map: *const MapDef, key: u32, flags: u64) c_long = @compileError("TODO");
...
pub const sk_redirect_map: fn (skb: *SkBuff, map: *const MapDef, key: u32, flags: u64) !void = @compileError("TODO");

/// Add an entry to, or update a *map* referencing sockets. The *skops* is used
/// as a new value for the entry associated to *key*. *flags* is one of:
///
/// **BPF_NOEXIST**
/// 	The entry for *key* must not exist in the map.
/// **BPF_EXIST**
...
pub const sock_map_update: fn (skops: *SockOps, map: *const MapDef, key: *u8, flags: u64) !void = @compileError("TODO");

/// Adjust the address pointed by *xdp_md*\ **->data_meta** by *delta* (which
/// can be positive or negative). Note that this operation modifies the address
/// stored in *xdp_md*\ **->data**, so the latter must be loaded only after the
/// helper has been called.
///
/// The use of *xdp_md*\ **->data_meta** is optional and programs are not
...
/// remains possible to set from XDP the **mark** or **priority** pointers, or
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const xdp_adjust_meta: fn (xdp_md: *XdpBuff, delta: i32) !void = @compileError("TODO");
...
///
pub const perf_event_read_value: fn (map: *const MapDef, flags: u64, buf: []u8) !void = @compileError("TODO");

/// For en eBPF program attached to a perf event, retrieve the value of the
/// event counter associated to *ctx* and store it in the structure pointed by
/// *buf* and of size *buf_size*. Enabled and running times are also stored in
/// the structure (see description of helper **bpf_perf_event_read_value**\ ()
/// for more details).
pub const perf_prog_read_value: fn (ctx: *PerfEventData, buf: []u8) !void = @compileError("TODO");

/// Emulate a call to **getsockopt()** on the socket associated to *bpf_socket*,
/// which must be a full socket. The *level* at which the option resides and the
/// name *optname* of the option must be specified, see **getsockopt(2)** for
/// more information.  The retrieved value is stored in the structure pointed by
...
/// * **IPPROTO_IP**, which supports *optname* **IP_TOS**.
/// * **IPPROTO_IPV6**, which supports *optname* **IPV6_TCLASS**.
pub const getsockopt: fn (socket: anytype, level: i32, optname: i32, opt: []u8) !void = @compileError("TODO");
...
/// **CONFIG_BPF_KPROBE_OVERRIDE** configuration option, and in this case it
/// only works on functions tagged with **ALLOW_ERROR_INJECTION** in the kernel
/// code.
///
...
pub const override_return: fn (regs: *PtRegs, rc: u64) !void = @compileError("TODO");

/// Attempt to set the value of the **bpf_sock_ops_cb_flags** field for the full
/// TCP socket associated to *bpf_sock_ops* to *argval*.
///
/// The primary use of this field is to determine if there should be calls to
/// eBPF programs of type **BPF_PROG_TYPE_SOCK_OPS** at various points in the
/// TCP code. A program of the same type can change its value, per connection
/// and as necessary, when the connection is established. This field is directly
...
///
/// Return code **-EINVAL** if the socket is not a full TCP socket; otherwise, a
...
pub const sock_ops_cb_flags: fn (sock: *SockOps, argval: i32) c_long = @compileError("TODO");
...
/// to make the distinction (ingress path is selected if the flag is present,
/// egress path otherwise). This is the only flag supported for now.
pub const msg_redirect_map: fn (msg: *SkMsgBuff, map: *const MapDef, key: u32, flags: u64) !void = @compileError("TODO");
...
///   logical messages that the eBPF program is supposed to read and for which
...
/// next *bytes*. If *bytes* is smaller than the current data being processed
/// from a **sendmsg**\ () or **sendfile**\ () system call, the first *bytes*
/// will be sent and the eBPF program will be re-run with the pointer for start
/// of data pointing to byte number *bytes* **+ 1**. If *bytes* is larger than
/// the current data being processed, then the eBPF verdict will be applied to
...
pub const msg_apply_bytes: fn (msg: *SkMsgBuff, bytes: u32) !void = @compileError("TODO");
...
/// needs *bytes* bytes to validate a header, this helper can be used to prevent
/// the eBPF program to be called again until *bytes* have been accumulated.
pub const msg_cork_bytes: fn (msg: *SkMsgBuff, bytes: u32) !void = @compileError("TODO");

/// For socket policies, pull in non-linear data from user space for *msg* and
/// set pointers *msg*\ **->data** and *msg*\ **->data_end** to *start* and
/// *end* bytes offsets into *msg*, respectively.
///
...
/// For **sendmsg**\ () hooks this is likely the first scatterlist element. But
/// for calls relying on the **sendpage** handler (e.g. **sendfile**\ ()) this
/// will be the range (**0**, **0**) because the data is shared with user space
/// and by default the objective is to avoid allowing user space to modify data
/// while (or after) eBPF verdict is being decided. This helper can be used to
/// pull in data and to set the start and end pointer to given values. Data will
/// be copied if necessary (i.e. if data was not linear and if start and end
...
/// All values for *flags* are reserved for future usage, and must be left at
/// zero.
pub const msg_pull_data: fn (msg: *SkMsgBuff, start: u32, end: u32, flags: u64) !void = @compileError("TODO");

/// Adjust (move) *xdp_md*\ **->data_end** by *delta* bytes. It is possible to
/// both shrink and grow the packet tail.  Shrink done via *delta* being a
/// negative integer.
///
/// A call to this helper is susceptible to change the underlying packet buffer.
/// Therefore, at load time, all checks on pointers previously done by the
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const xdp_adjust_tail: fn (xdp_md: *XdpBuff, delta: i32) !void = @compileError("TODO");
...
pub const skb_get_xfrm_state: fn (skb: *SkBuff, index: u32, xfrm_state: *XfrmStat, size: u32, flags: u64) !void = @compileError("TODO");
...
///
/// **bpf_get_stack**\ () can collect up to **PERF_MAX_STACK_DEPTH** both kernel
...
///
/// 	# sysctl kernel.perf_event_max_stack=<new value>
/// Returns a non-negative value equal to or less than *size* on success, or a
/// negative error in case of failure.
pub const get_stack: fn (ctx: anytype, buf: []u8, flags: u64) usize = @compileError("TODO");

/// This helper is similar to **bpf_skb_load_bytes**\ () in that it provides an
/// easy way to load *len* bytes from *offset* from the packet associated to
/// *skb*, into the buffer pointed by *to*. The difference to
/// **bpf_skb_load_bytes**\ () is that a fifth argument *start_header* exists in
/// order to select a base offset to start from. *start_header* can be one of:
///
/// **BPF_HDR_START_MAC**
...
/// *skb*\ **->data** does not always point to the start of the mac header and
/// where "direct packet access" is not available.
pub const skb_load_bytes_relative: fn (skb: anytype, offset: u32, to: []u8, start_header: u32) !void = @compileError("TODO");
...
///	*flags* argument can be a combination of one or more of the following
///	values:
///
...
///
///	*ctx* is either **struct xdp_md** for XDP programs or **struct sk_buff** tc
///	cls_act programs.
///
///	Return
...
pub const fib_lookup: fn (ctx: anytype, params: *FibLookup, plen: i32, flags: u32) c_long = @compileError("TODO");

///	Add an entry to, or update a sockhash *map* referencing sockets.  The
///	*skops* is used as a new value for the entry associated to *key*. *flags* is
///	one of:
///
///	**BPF_NOEXIST**
///		The entry for *key* must not exist in the map.
...
///	by the socket being added. If the socket is already attached to eBPF
///	programs, this results in an error.
pub const sock_hash_update: fn (skops: *SockOps, map: *const MapDef, key: anytype, flags: u64) !void = @compileError("TODO");
...
///	*flags* is used to make the distinction (ingress path is selected if the
///	flag is present, egress path otherwise). This is the only flag supported for
///	now.
///
///	Return **SK_PASS** on success, or **SK_DROP** on error.
pub const msg_redirect_hash: fn (msg: *SkMsgBuff, map: *const MapDef, key: anytype, flags: u64) !void = @compileError("TODO");
...
///	program returns **SK_PASS**), redirect it to the socket referenced by *map*
///	(of type **BPF_MAP_TYPE_SOCKHASH**) using hash *key*. Both ingress and
///	egress interfaces can be used for redirection. The **BPF_F_INGRESS** value
///	in *flags* is used to make the distinction (ingress path is selected if the
///	flag is present, egress otherwise). This is the only flag supported for now.
///
///	Return **SK_PASS** on success, or **SK_DROP** on error.
pub const sk_redirect_hash: fn (skb: *SkBuff, map: *const MapDef, key: anytype, flags: u64) !void = @compileError("TODO");

///	Encapsulate the packet associated to *skb* within a Layer 3 protocol header.
///	This header is provided in the buffer at address *hdr*, with *len* its size
///	in bytes. *type* indicates the protocol of the header and can be one of:
///
///	**BPF_LWT_ENCAP_SEG6**
...
///		note that if **skb_is_gso**\ (*skb*) is true, no more than two headers
///		can be prepended, and the inner header, if present, should be either GRE
///		or UDP/GUE.
///
///	**BPF_LWT_ENCAP_SEG6**\ \* types can be called by BPF programs of type
///	**BPF_PROG_TYPE_LWT_IN**; **BPF_LWT_ENCAP_IP** type can be called by bpf
///	programs of types **BPF_PROG_TYPE_LWT_IN** and **BPF_PROG_TYPE_LWT_XMIT**.
///
/// A call to this helper is susceptible to change the underlying packet buffer.
/// Therefore, at load time, all checks on pointers previously done by the
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const lwt_push_encap: fn (skb: *SkBuff, type: u32, hdr: []u8) !void = @compileError("TODO");

///	Store *len* bytes from address *from* into the packet associated to *skb*,
///	at *offset*. Only the flags, tag and TLVs inside the outermost IPv6 Segment
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const lwt_seg6_store_bytes: fn (skb: *SkBuff, offset: u32, from: []const u8) !void = @compileError("TODO");
...
///	*delta* bytes. Only offsets after the segments are accepted. *delta* can be
///	as well positive (growing) as negative (shrinking).
///
/// A call to this helper is susceptible to change the underlying packet buffer.
/// Therefore, at load time, all checks on pointers previously done by the
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const lwt_seg6_adjust_srh: fn (skb: *SkBuff, offset: u32, delta: i32) !void = @compileError("TODO");

///	Apply an IPv6 Segment Routing action of type *action* to the packet
///	associated to *skb*. Each action takes a parameter contained at address
...
/// verifier are invalidated and must be performed again, if the helper is used
/// in combination with direct packet access.
pub const lwt_seg6_action: fn (skb: *SkBuff, action: u32, param: []u8) !void = @compileError("TODO");
...
pub const rc_repeat: fn (ctx: anytype) !void = @compileError("TODO");

///	This helper is used in programs implementing IR decoding, to report a
///	successfully decoded key press with *scancode*, *toggle* value in the given
///	*protocol*. The scancode will be translated to a keycode using the rc
///	keymap, and reported as an input key down event. After a period a key up
...
///	The *ctx* should point to the lirc sample as passed into the program.
///
///	The *protocol* is the decoded protocol number (see **enum rc_proto** for
///	some predefined values).
///
///	This helper is only available is the kernel was compiled with the
///	**CONFIG_BPF_LIRC_MODE2** configuration option set to "**y**".
pub const rc_keydown: fn (ctx: anytype, protocol: u32, scancode: u64, toggle: u32) !void = @compileError("TODO");

/// Return the cgroup v2 id of the socket associated with the *skb*.  This is
/// roughly similar to the **bpf_get_cgroup_classid**\ () helper for cgroup v1
...
pub const skb_cgroup_id: fn (skb: *SkBuff) ?u64 = @compileError("TODO");

/// Returns a 64-bit integer containing the current cgroup id based on the
/// cgroup within which the current task is running.
pub const get_current_cgroup_id = helpers.get_current_cgroup_id;

///	Get the pointer to the local storage area.  The type and the size of the
///	local storage is defined by the *map* argument.  The *flags* meaning is
...
pub const get_local_storage: fn (map: anytype, flags: u64) []u8 = @compileError("TODO");

///	Select a **SO_REUSEPORT** socket from a **BPF_MAP_TYPE_REUSEPORT_ARRAY**
///	*map*.  It checks the selected socket is matching the incoming request in
///	the socket buffer.
pub const sk_select_reuseport: fn (reuse: *SkReuseportMd, map: *const MapDef, key: anytype, flags: u64) !void = @compileError("TODO");

///	Return id of cgroup v2 that is ancestor of cgroup associated with the *skb*
///	at the *ancestor_level*.  The root cgroup is at *ancestor_level* zero and
///	each step down the hierarchy increments the level. If *ancestor_level* ==
///	level of cgroup associated with *skb*, then return value will be same as
...
///	**bpf_skb_cgroup_id**\ ().
pub const skb_ancestor_cgroup_id: fn (skb: *SkBuff, ancestor_level: i32) ?u64 = @compileError("TODO");

///	Look for TCP socket matching *tuple*, optionally in a child network
///	namespace *netns*. The return value must be checked, and if non-**NULL**,
///	released via **bpf_sk_release**\ ().
///
...
///	table in the netns associated with the *ctx* will be used. For the TC hooks,
///	this is the netns of the device in the skb. For socket hooks, this is the
///	netns of the socket.  If *netns* is any other signed 32-bit value greater
...
pub const sk_lookup_tcp: fn (ctx: anytype, tuple: *SockTuple, tuple_size: u32, netns: u64, flags: u64) ?*Sock = @compileError("TODO");
...
///		sockets with reuseport option, the **struct bpf_sock** result is from
///		*reuse*\ **->socks**\ [] using the hash of the tuple.
pub const sk_lookup_udp: fn (ctx: anytype, tuple: *SockTuple, tuple_size: u32, netns: u64, flags: u64) ?*Sock = @compileError("TODO");

///	Release the reference held by *sock*. *sock* must be a non-**NULL** pointer
///	that was returned from **bpf_sk_lookup_xxx**\ ().
pub const sk_release: fn (sock: *Sock) !void = @compileError("TODO");
...
/// 	for this.
pub const map_push_elem: fn (map: *const MapDef, value: anytype, flags: u64) !void = @compileError("TODO");

/// Pop an element from *map*.
pub const map_pop_elem: fn (map: *const MapDef, value: anytype) !void = @compileError("TODO");

/// Get an element from *map* without removing it.
pub const map_peek_elem: fn (map: *const MapDef, value: anytype) !void = @compileError("TODO");

///	For socket policies, insert *len* bytes into *msg* at offset *start*.
///
///	If a program of type **BPF_PROG_TYPE_SK_MSG** is run on a *msg* it may want
///	to insert metadata or options into the *msg*.  This can later be read and
///	used by any of the lower layer BPF hooks.
///
...
///	to handle them.
pub const msg_push_data: fn (msg: *SkMsgBuff, start: u32, len: u32, flags: u64) !void = @compileError("TODO");

///	Will remove *len* bytes from a *msg* starting at byte *start*.  This may
///	result in **ENOMEM** errors under certain situations if an allocation and
...
///	parameters are invalid either due to *start* byte not being valid part of
///	*msg* payload and/or *pop* value being to large.
pub const msg_pop_data: fn (msg: *SkMsgBuff, start: u32, len: u32, flags: u64) !void = @compileError("TODO");
...
///	This helper is only available is the kernel was compiled with the
///	**CONFIG_BPF_LIRC_MODE2** configuration option set to "**y**".
pub const rc_pointer_rel: fn (ctx: anytype, rel_x: i32, rel_y: i32) !void = @compileError("TODO");
...
///	with a call to **bpf_spin_unlock**\ (\ *lock*\ ).
///
...
///	  allowed.
///	* The **BPF_LD_ABS** and **BPF_LD_IND** instructions are not allowed inside
///	  a spinlock-ed region.
...
///	  storing data into the **struct bpf_spin_lock** *lock*\ **;** field of a
///	  map is not allowed.
///	* To use the **bpf_spin_lock**\ () helper, the BTF description of the map
///	  value must be a struct and have **struct bpf_spin_lock** *anyname*\ **;**
///	  field at the top level.  Nested lock inside another struct is not allowed.
///	* The **struct bpf_spin_lock** *lock* field in a map value must be aligned
///	  on a multiple of 4 bytes in that value.
...
///	* **bpf_spin_lock** cannot be on the stack or inside a networking packet (it
///	  can only be inside of a map values).
///	* **bpf_spin_lock** is available to root only.
///	* Tracing programs and socket filter programs cannot use **bpf_spin_lock**\
///	  () due to insufficient preemption checks (but this may change in the
///	  future).
///	* **bpf_spin_lock** is not allowed in inner maps of map-in-map.
pub const spin_lock: fn (lock: *SpinLock) !void = @compileError("TODO");

///	Release the *lock* previously locked by a call to **bpf_spin_lock**\ (\
///	*lock*\ ).
pub const spin_unlock: fn (lock: *SpinLock) !void = @compileError("TODO");

///	This helper gets a **struct bpf_sock** pointer such that all the fields in
///	this **bpf_sock** can be accessed.
pub const sk_fullsock: fn (sk: *Sock) ?*Sock = @compileError("TODO");

///	This helper gets a **struct bpf_tcp_sock** pointer from a **struct
///	bpf_sock** pointer.
pub const tcp_sock: fn (sk: *Sock) ?*TcpSock = @compileError("TODO");
...
pub const skb_ecn_set_ce: fn (skb: *SkBuff) bool = @compileError("TODO");

///	Return a **struct bpf_sock** pointer in **TCP_LISTEN** state.
///	**bpf_sk_release**\ () is unnecessary and not allowed.
pub const get_listener_sock: fn (sk: *Sock) ?*Sock = @compileError("TODO");
...
///	returns timewait or request sockets. Use **bpf_sk_fullsock**\ () or
///	**bpf_tcp_sock**\ () to access the full structure.
///
///	This helper is available only if the kernel was compiled with **CONFIG_NET**
///	configuration option.
///
///	Return Pointer to **struct bpf_sock**, or **NULL** in case of failure.  For
///	sockets with reuseport option, the **struct bpf_sock** result is from
///	*reuse*\ **->socks**\ [] using the hash of the tuple.
pub const skc_lookup_tcp: fn (ctx: anytype, tuple: *SockTuple, tuple_size: u32, netns: u64, flags: u64) ?*Sock = @compileError("TODO");

/// Check whether *iph* and *th* contain a valid SYN cookie ACK for the
/// listening socket in *sk*.
///
/// *iph* points to the start of the IPv4 or IPv6 header, while *iph_len*
/// contains **sizeof**\ (**struct iphdr**) or **sizeof**\ (**struct ip6hdr**).
///
/// *th* points to the start of the TCP header, while *th_len* contains
/// **sizeof**\ (**struct tcphdr**).
///
/// Return 0 if *iph* and *th* are a valid SYN cookie ACK, or a negative error
/// otherwise.
pub const tcp_check_syncookie: fn (sk: *Sock, iph: []u8, th: *TcpHdr, th_len: u32) !void = @compileError("TODO");
...
///	**-E2BIG** if the buffer wasn't big enough (*buf* will contain truncated
///	name in this case).
pub const sysctl_get_name: fn (ctx: *SysCtl, buf: []u8, flags: u64) !usize = @compileError("TODO");
...
///	The buffer is always NUL terminated, unless it's zero-sized.
///
///	Returns number of character copied (not including the trailing NUL).
///	**-E2BIG** if the buffer wasn't big enough (*buf* will contain truncated
///	name in this case).  **-EINVAL** if current value was unavailable, e.g.
///	because sysctl is uninitialized and read returns -EIO for it.
pub const sysctl_get_current_value: fn (ctx: *SysCtl, buf: []u8) !usize = @compileError("TODO");

///	Get new value being written by user space to sysctl (before the actual write
///	happens) and copy it as a string into provided by program buffer *buf* of
///	size *buf_len*.
///
...
///	Returns number of character copied (not including the trailing NUL).
///	**-E2BIG** if the buffer wasn't big enough (*buf* will contain truncated
///	name in this case).  **-EINVAL** if sysctl is being read.
pub const sysctl_get_new_value: fn (ctx: *SysCtl, buf: []u8) !usize = @compileError("TODO");

///	Override new value being written by user space to sysctl with value provided
///	by program in buffer *buf* of size *buf_len*.
///
///	*buf* should contain a string in same form as provided by user space on
///	sysctl write.
///
///	User space may write new value at file position > 0. To override the whole
///	sysctl value file position should be set to zero.
///
///	Returns 0 on success.  **-E2BIG** if the *buf_len* is too big.  **-EINVAL**
///	if sysctl is being read.
pub const sysctl_set_new_value: fn (ctx: *SysCtl, buf: []u8) !void = @compileError("TODO");

///	Convert the initial part of the string from buffer *buf* of size *buf_len*
...
///	user space **strtol**\ (3).
///
///	Return Number of characters consumed on success. Must be positive but no
///	more than *buf_len*.  **-EINVAL** if no valid digits were found or
///	unsupported base was provided.  **-ERANGE** if resulting value was out of
///	range.
pub const strtol: fn (buf: []const u8, flags: u64, res: *c_long) !usize = @compileError("TODO");
...
///	unsupported base was provided.  **-ERANGE** if resulting value was out of
///	range.
pub const strtoul: fn (buf: []const u8, flags: u64, res: *c_ulong) !usize = @compileError("TODO");

///	Get a bpf-local-storage from a *sk*.
///
///	Logically, it could be thought of getting the value from a *map* with *sk*
///	as the **key**.  From this perspective,  the usage is not much different
///	from **bpf_map_lookup_elem**\ (*map*, **&**\ *sk*) except this helper
///	enforces the key must be a full socket and the map must be a
///	**BPF_MAP_TYPE_SK_STORAGE** also.
///
///	Underneath, the value is stored locally at *sk* instead of the *map*.  The
///	*map* is used as the bpf-local-storage "type". The bpf-local-storage "type"
///	(i.e. the *map*) is searched against all bpf-local-storages residing at
///	*sk*.
///
///	An optional *flags* (**BPF_SK_STORAGE_GET_F_CREATE**) can be used such that
///	a new bpf-local-storage will be created if one does not exist.  *value* can
///	be used together with **BPF_SK_STORAGE_GET_F_CREATE** to specify the initial
///	value of a bpf-local-storage.  If *value* is **NULL**, the new
...
pub const sk_storage_get: fn (map: *const MapDef, sk: *Sock, value: anytype, flags: u64) ?[]u8 = @compileError("TODO");

///	Delete a bpf-local-storage from a *sk*.
///
///	Return 0 on success.  **-ENOENT** if the bpf-local-storage cannot be found.
pub const sk_storage_delete: fn (map: *const MapDef, sk: *Sock) !void = @compileError("TODO");

///	Send signal *sig* to the process of the current task.  The signal may be
///	delivered to any of this process's threads.
///
///	Return
...
pub const send_signal: fn (sig: u32) !void = @compileError("TODO");
...
pub const tcp_get_syncookie: fn (sk: *Sock, iph: []u8, th: *TcpHdr, th_len: u32) !u64 = @compileError("TODO");
...
pub const skb_output: fn (ctx: anytype, map: *const MapDef, flags: u64, data: []u8) !void = @compileError("TODO");

/// Safely attempt to read *size* bytes from user space address *unsafe_ptr* and
/// store the data in *dst*.
pub const probe_read_user: fn (dst: []u8, unsafe: [*]const u8) !void = @compileError("TODO");

/// Safely attempt to read *size* bytes from kernel space address *unsafe_ptr*
/// and store the data in *dst*.
pub const probe_read_kernel: fn (dst: []u8, unsafe: [*]const u8) !void = @compileError("TODO");

/// Copy a NUL terminated string from an unsafe user address *unsafe_ptr* to
...
pub const probe_read_user_str: fn (dst: []u8, unsafe: [*:0]const u8) !usize = @compileError("TODO");

/// Copy a NUL terminated string from an unsafe kernel address *unsafe_ptr* to
/// *dst*. Same semantics as with **bpf_probe_read_user_str**\ () apply.
///
/// Return On success, the strictly positive length of the string, including the
/// trailing NUL character. On error, a negative value.
pub const probe_read_kernel_str: fn (dst: []u8, unsafe: [*:0]const u8) !usize = @compileError("TODO");

///	Send out a tcp-ack. *tp* is the in-kernel struct **tcp_sock**.  *rcv_nxt* is
///	the ack_seq to be sent out.
pub const tcp_send_ack: fn (tp: anytype, rcv_nxt: u32) !void = @compileError("TODO");
...
///		**-EPERM** if no permission to send the *sig*.
///		**-EAGAIN** if bpf program can try again.
pub const send_signal_thread: fn (sig: u32) !void = @compileError("TODO");

///	Obtain the 64bit jiffies
pub const jiffies64 = helpers.jiffies64;

///	For an eBPF program attached to a perf event, retrieve the branch records
///	(**struct perf_branch_entry**) associated to *ctx* and store it in the
///	buffer pointed by *buf* up to size *size* bytes.
///
///	Return on success, number of bytes written to *buf*. On error, a negative
///	value.
///
///	The *flags* can be set to **BPF_F_GET_BRANCH_RECORDS_SIZE** to instead
///	return the number of bytes required to store all the branch entries. If this
///	flag is set, *buf* may be NULL.
///
///	**-EINVAL** if arguments invalid or **size** not a multiple of **sizeof**\
///	(**struct perf_branch_entry**\ ).
///
///	**-ENOENT** if architecture does not support branch records.
pub const read_branch_records: fn (ctx: *PerfEventData, buf: []u8, flags: u64) !usize = @compileError("TODO");
...
pub const get_ns_current_pid_tgid: fn (dev: u64, ino: u64, nsdata: PidNsInfo, size: u32) !void = @compileError("TODO");
...
///	raw_tracepoint bpf programs.
pub const xdp_output: fn (ctx: anytype, map: *const MapDef, flags: u64, data: []u8) !void = @compileError("TODO");

/// Retrieve the cookie (generated by the kernel) of the network namespace the
/// input *ctx* is associated with. The network namespace cookie remains stable
/// for its lifetime and provides a global identifier that can be assumed
/// unique. If *ctx* is NULL, then the helper returns the cookie for the initial
/// network namespace. The cookie itself is very similar to that of
/// **bpf_get_socket_cookie**\ () helper, but for network namespaces instead of
/// sockets.
///
/// Returns a 8-byte long opaque number.
pub const get_netns_cookie: fn (ctx: anytype) u64 = @compileError("TODO");

/// Return id of cgroup v2 that is ancestor of the cgroup associated with the
/// current task at the *ancestor_level*. The root cgroup is at *ancestor_level*
/// zero and each step down the hierarchy increments the level. If
/// *ancestor_level* == level of cgroup associated with the current task, then
/// return value will be the same as that of **bpf_get_current_cgroup_id**\ ().
///
/// The helper is useful to implement policies based on cgroups that are upper
...
pub const get_current_ancestor_cgroup_id: fn (ancestor_level: i32) ?u64 = @compileError("TODO");

///	Helper is overloaded depending on BPF program type. This description applies
///	to **BPF_PROG_TYPE_SCHED_CLS** and **BPF_PROG_TYPE_SCHED_ACT** programs.
///
///	Assign the *sk* to the *skb*. When combined with appropriate routing
///	configuration to receive the packet towards the socket, will cause *skb* to
///	be delivered to the specified socket.  Subsequent redirection of *skb* via
///	**bpf_redirect**\ (), **bpf_clone_redirect**\ () or other methods outside of
...
///		(reuseport).
pub const sk_assign: fn (skb: *SkBuff, sk: *Sock, flags: u64) !void = @compileError("TODO");
...
///	packet description provided by the *ctx* object.
///
///	L4 protocol (**IPPROTO_TCP** or **IPPROTO_UDP**) must be an exact match.
///	While IP family (**AF_INET** or **AF_INET6**) must be compatible, that is
///	IPv6 sockets that are not v6-only can be selected for IPv4 packets.
///
///	Only TCP listeners and UDP unconnected sockets can be selected. *sk* can
///	also be NULL to reset any previous selection.
///
///	*flags* argument can combination of following values:
///
///	* **BPF_SK_LOOKUP_F_REPLACE** to override the previous socket selection,
///	  potentially done by a BPF program that ran before us.
///
///	* **BPF_SK_LOOKUP_F_NO_REUSEPORT** to skip load-balancing within reuseport
...
///		* **-EAFNOSUPPORT** if socket family (*sk->family*) is not compatible
///		  with packet family (*ctx->family*).
///		* **-EEXIST** if socket has been already selected, potentially by
///		  another program, and **BPF_SK_LOOKUP_F_REPLACE** flag was not specified.
///		* **-EINVAL** if unsupported flags were specified.
///		* **-EPROTOTYPE** if socket L4 protocol (*sk->protocol*) doesn't match
///		  packet protocol (*ctx->protocol*).
///		* **-ESOCKTNOSUPPORT** if socket is not in allowed state (TCP listening
///		  or UDP unconnected).
pub const sk_assign: fn (ctx: *SkLookup, sk: *Sock, flags: u64) !void = @compileError("TODO");
...
pub const ktime_get_boot_ns = helpers.ktime_get_boot_ns;

/// **bpf_seq_printf**\ () uses seq_file **seq_printf**\ () to print out the
/// format string.  The *m* represents the seq_file. The *fmt* and *fmt_size*
/// are for the format string itself. The *data* and *data_len* are format
/// string arguments. The *data* are a **u64** array and corresponding format
...
///
///	Formats **%s**, **%p{i,I}{4,6}** requires to read kernel memory.  Reading
...
///	for **%s** will be an empty string, and the ip address for **%p{i,I}{4,6}**
...
///		again.
pub const seq_printf: fn (m: *SeqFile, fmt: []const u8, data: []const u8) !void = @compileError("TODO");

/// **bpf_seq_write**\ () uses seq_file **seq_write**\ () to write the data.
/// The *m* represents the seq_file. The *data* and *len* represent the data to
/// write in bytes.
///
/// Return
/// 	0 on success, or a negative error in case of failure:
///		**-EOVERFLOW** if an overflow happened: The same object will be tried again.
pub const seq_write: fn (m: *SeqFile, data: []const u8) !void = @compileError("TODO");

///	Return the cgroup v2 id of the socket *sk*.
///
///	*sk* must be a non-**NULL** pointer to a full socket, e.g. one returned from
///	**bpf_sk_lookup_xxx**\ (), **bpf_sk_fullsock**\ (), etc. The format of
///	returned id is same as in **bpf_skb_cgroup_id**\ ().
///
...
pub const sk_cgroup_id: fn (sk: *Sock) ?u64 = @compileError("TODO");

///	Return id of cgroup v2 that is ancestor of cgroup associated with the *sk*
///	at the *ancestor_level*.  The root cgroup is at *ancestor_level* zero and
///	each step down the hierarchy increments the level. If *ancestor_level* ==
///	level of cgroup associated with *sk*, then return value will be same as that
///	of **bpf_sk_cgroup_id**\ ().
///
///	The helper is useful to implement policies based on cgroups that are upper
///	in hierarchy than immediate cgroup associated with *sk*.
///
///	The format of returned id and helper limitations are same as in
///	**bpf_sk_cgroup_id**\ ().
///
...
pub const sk_ancestor_cgroup_id: fn (sk: *Sock, ancestor_level: i32) ?u64 = @compileError("TODO");

/// Copy *size* bytes from *data* into a ring buffer *ringbuf*.  If
/// **BPF_RB_NO_WAKEUP** is specified in *flags*, no notification of new data
/// availability is sent.  If **BPF_RB_FORCE_WAKEUP** is specified in *flags*,
/// notification of new data availability is sent unconditionally.
pub const ringbuf_output: fn (ringbuf: anytype, data: []u8, flags: u64) !void = @compileError("TODO");

/// Reserve *size* bytes of payload in a ring buffer *ringbuf*.
///
/// Valid pointer with *size* bytes of memory available; NULL, otherwise.
pub const ringbuf_reserve: fn (ringbuf: anytype, size: u64, flags: u64) ?[]u8 = @compileError("TODO");
...
///
/// Always succeeds.
pub const ringbuf_submit: fn (data: anytype, flags: u64) void = @compileError("TODO");
...
///
/// Always succeeds.
pub const ringbuf_discard: fn (data: anytype, flags: u64) void = @compileError("TODO");

///	Query various characteristics of provided ring buffer. What exactly is
///	queries is determined by *flags*:
///
...
pub const ringbuf_query: fn (ringbuf: anytype, flags: u64) ?u64 = @compileError("TODO");
...
/// TCP | through **bpf_skb_adjust_room**\ () helper with passing in
/// **BPF_F_ADJ_ROOM_NO_CSUM_RESET** flag would require one	call to
/// **bpf_csum_level**\ () with **BPF_CSUM_LEVEL_DEC** since the UDP header is
...
///
/// * **BPF_CSUM_LEVEL_INC**: Increases skb->csum_level for skbs with
///   CHECKSUM_UNNECESSARY.
/// * **BPF_CSUM_LEVEL_DEC**: Decreases skb->csum_level for skbs with
///   CHECKSUM_UNNECESSARY.
/// * **BPF_CSUM_LEVEL_RESET**: Resets skb->csum_level to 0 and sets
...
/// 	CHECKSUM_UNNECESSARY.
pub const csum_level: fn (skb: *SkBuff, level: u64) !void = @compileError("TODO");

///	Dynamically cast a *sk* pointer to a *tcp6_sock* pointer.
pub const skc_to_tcp6: fn (sk: anytype) ?*Tcp6Sock = @compileError("TODO");

///	Dynamically cast a *sk* pointer to a *tcp_sock* pointer.
pub const skc_to_tcp_sock: fn (sk: anytype) ?*TcpSock = @compileError("TODO");

///	Dynamically cast a *sk* pointer to a *tcp_timewait_sock* pointer.
pub const skc_to_tcp_timewait_sock: fn (sk: anytype) ?*TcpTimewaitSock = @compileError("TODO");

///	Dynamically cast a *sk* pointer to a *tcp_request_sock* pointer.
pub const skc_to_tcp_request_sock: fn (sk: anytype) ?*TcpRequestSock = @compileError("TODO");

///	Dynamically cast a *sk* pointer to a *udp6_sock* pointer.
pub const skc_to_udp6_sock: fn (sk: anytype) ?*Udp6Sock = @compileError("TODO");

///	Return a user or a kernel stack in bpf program provided buffer.  To achieve
///	this, the helper needs *task*, which is a valid pointer to struct
///	task_struct. To store the stacktrace, the bpf program provides *buf* with
///	a nonnegative *size*.
///
///	The last argument, *flags*, holds the number of stack frames to skip (from 0
///	to 255), masked with **BPF_F_SKIP_FIELD_MASK**. The next bits can be used to
...
///		**BPF_F_USER_STACK** is also specified.
///
///	**bpf_get_task_stack**\ () can collect up to **PERF_MAX_STACK_DEPTH** both
///	kernel and user frames, subject to sufficient large buffer size. Note that
///	this limit can be controlled with the **sysctl** program, and that it should
...
pub const get_task_stack: fn (task: *TaskStruct, buf: []u8, flags: u64) !usize = @compileError("TODO");

...
const in_userspace = switch (std.builtin.arch) {
    .bpfel, .bpfeb => false,
    else => true,
};

fn assert_bpf_program() void {
    if (in_userspace) @compileError("BPF helpers are not available in userspace");
}
...
pub const PerfEventData = packed struct {
    ...
    pub fn read_branch_records(ctx: *PerfEventData, buf: []u8, flags: u64) !usize {
        assert_bpf_program();

        const rc = helpers.read_branch_records(ctx, buf.ptr, buf.len, flags);
        return switch (rc) {
            0...std.math.maxInt(@TypeOf(rc)) => rc,
            else => error.Unknown,
        };
    }
    ...
    /// for more details).
    pub fn perf_prog_read_value(ctx: *PerfEventData, buf: []u8) !void {
        assert_bpf_program();

        const rc = helpers.perf_prog_read_value(ctx, buf.ptr, buf.len);
        return switch (rc) {
            0 => {},
            else => error.Unknown,
        };
...
pub const SkBuff = packed struct {
    ...
    pub fn skb_get_xfrm_state(skb: *SkBuff, index: u32, xfrm_state: *XfrmStat, size: u32, flags: u64) !void {
        assert_bpf_program();
...
pub const SkMsgBuff = packed struct {
    ...
    /// to make the distinction (ingress path is selected if the flag is present,
    /// egress path otherwise). This is the only flag supported for now.
    pub fn msg_redirect_map(msg: *SkMsgBuff, map: *const SockMap, key: u32, flags: u64) !void {
        assert_bpf_program();
    ...
    pub fn msg_apply_bytes(msg: *SkMsgBuff, bytes: u32) !void {
        assert_bpf_program();
    ...
    pub fn msg_cork_bytes(msg: *SkMsgBuff, bytes: u32) !void {
        assert_bpf_program();
    ...
    /// All values for *flags* are reserved for future usage, and must be left at
    /// zero.
    pub fn msg_pull_data(msg: *SkMsgBuff, start: u32, end: u32, flags: u64) !void {
        assert_bpf_program();
    ...
    ///
    ///	Return **SK_PASS** on success, or **SK_DROP** on error.
    pub fn msg_redirect_hash(msg: *SkMsgBuff, map: *const SockHash, key: anytype, flags: u64) !void {
        assert_bpf_program();
    ...
    ///	to handle them.
    pub fn msg_push_data(msg: *SkMsgBuff, start: u32, len: u32, flags: u64) !void {
        assert_bpf_program();
    ...
    pub fn msg_pop_data(msg: *SkMsgBuff, start: u32, len: u32, flags: u64) !void {
        assert_bpf_program();
...
pub const Sock = packed struct {
    ...
    pub fn sk_cgroup_id(sk: *Sock) ?u64 {
        assert_bpf_program();
    ...
    pub fn sk_ancestor_cgroup_id(sk: *Sock, ancestor_level: i32) ?u64 {
        assert_bpf_program();
    ...
    pub fn sk_release(sock: *Sock) !void {
        assert_bpf_program();
    ...
    pub fn sk_fullsock(sk: *Sock) ?*Sock {
        assert_bpf_program();
    ...
    ///	bpf_sock** pointer.
    pub fn tcp_sock(sk: *Sock) ?*TcpSock {
        assert_bpf_program();
    ...
    pub fn get_listener_sock(sk: *Sock) ?*Sock {
        assert_bpf_program();
...
pub const SockOps = packed struct {
    ...
    pub fn sock_map_update(skops: *SockOps, map: *const MapDef, key: *u8, flags: u64) !void {
        assert_bpf_program();
    ...
    pub fn sock_ops_cb_flags(sock: *SockOps, argval: i32) c_long {
        assert_bpf_program();
    ...
    ///	by the socket being added. If the socket is already attached to eBPF
    ///	programs, this results in an error.
    pub fn sock_hash_update(skops: *SockOps, map: *const MapDef, key: anytype, flags: u64) !void {
        assert_bpf_program();
    ...
    /// * **IPPROTO_IP**, which supports *optname* **IP_TOS**.
    /// * **IPPROTO_IPV6**, which supports *optname* **IPV6_TCLASS**.
    pub fn setsockopt(socket: anytype, level: i32, opname: i32, opt: []u8) !void {
        assert_bpf_program();
    ...
    /// * **IPPROTO_IP**, which supports *optname* **IP_TOS**.
    /// * **IPPROTO_IPV6**, which supports *optname* **IPV6_TCLASS**.
    pub fn getsockopt(socket: anytype, level: i32, optname: i32, opt: []u8) !void {
        assert_bpf_program();
...
pub const SockTuple = packed union {
    ...
    pub fn sk_lookup_tcp(ctx: anytype, tuple: *SockTuple, tuple_size: u32, netns: u64, flags: u64) ?*Sock {
        assert_bpf_program();
    ...
    ///		sockets with reuseport option, the **struct bpf_sock** result is from
    ///		*reuse*\ **->socks**\ [] using the hash of the tuple.
    pub fn sk_lookup_udp(ctx: anytype, tuple: *SockTuple, tuple_size: u32, netns: u64, flags: u64) ?*Sock {
        assert_bpf_program();
...
pub const SysCtl = packed struct {
    ...
    ///	name in this case).
    pub fn sysctl_get_name(ctx: *SysCtl, buf: []u8, flags: u64) !usize {
        assert_bpf_program();
    ...
    pub fn sysctl_get_current_value(ctx: *SysCtl, buf: []u8) !usize {
        assert_bpf_program();
    ...
    pub fn sysctl_get_new_value(ctx: *SysCtl, buf: []u8) !usize {
        assert_bpf_program();
    ...
    pub fn sysctl_set_new_value(ctx: *SysCtl, buf: []u8) !void {
        assert_bpf_program();
...
pub const SeqFile = packed struct {
    ...
    ///		again.
    pub fn seq_printf(m: *SeqFile, fmt: []const u8, data: []const u8) !void {
        assert_bpf_program();
    ...
    pub fn seq_write(m: *SeqFile, data: []const u8) !void {
        assert_bpf_program();
...
pub const StackTrace = packed struct {
    ...
    /// case of failure.
    pub fn get_stackid(ctx: anytype, map: *const MapDef, flags: u64) c_long {
        assert_bpf_program();
...
pub const CGroupArray = packed struct {
    ...
    /// 	* A negative error code, if an error occurred.
    pub fn current_task_under_cgroup(map: *const MapDef, index: u32) !bool {
        assert_bpf_program();
...
pub const Ringbuf = packed struct {
    ...
    pub fn ringbuf_output(ringbuf: anytype, data: []u8, flags: u64) !void {
        assert_bpf_program();
    ...
    pub fn ringbuf_reserve(ringbuf: anytype, size: u64, flags: u64) ?[]u8 {
        assert_bpf_program();
    ...
    ///
    /// Always succeeds.
    pub fn ringbuf_submit(data: anytype, flags: u64) void {
        assert_bpf_program();
    ...
    ///
    /// Always succeeds.
    pub fn ringbuf_discard(data: anytype, flags: u64) void {
        assert_bpf_program();
    ...
    pub fn ringbuf_query(ringbuf: anytype, flags: u64) ?u64 {
        assert_bpf_program();
...
/// instead.
pub fn probe_read(dst: []u8, src: []u8) !void {
    assert_bpf_program();
...
pub fn probe_write_user(dst: *u8, src: []u8) !void {
    assert_bpf_program();
...
pub fn probe_read_str(dst: []u8, unsafe: [*:0]const u8) !usize {
    assert_bpf_program();
...
pub fn probe_read_user(dst: []u8, unsafe: [*]const u8) !void {
    assert_bpf_program();
...
pub fn probe_read_kernel(dst: []u8, unsafe: [*]const u8) !void {
    assert_bpf_program();
...
/// Return On success, the strictly positive length of the string, including the
/// trailing NUL character. On error, a negative value.
pub fn probe_read_kernel_str(dst: []u8, unsafe: [*:0]const u8) !usize {
    assert_bpf_program();
...
pub fn trace_printk(fmt: []const u8, args: anytype) !usize {
    assert_bpf_program();
...
pub const get_prandom_u32 = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
    get_prandom_u32;
...
pub const get_smp_processor_id = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
...
/// 	*current_task*\ **->pid**.
pub const get_current_pid_tgid = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
...
pub const get_current_uid_gid = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
    helpers.get_current_uid_gid;
...
pub fn redirect(ifindex: u32, flags: u64) !void {
    assert_bpf_program();
...
pub fn csum_diff(from: []u8, to: []u8, seed: u32) u64 {
    assert_bpf_program();
...
pub const get_current_task = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
    helpers.get_current_task;
...
pub const get_numa_node_id = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
    helpers.get_numa_node_id;
...
pub fn redirect_map(map: *const MapDef, key: u32, flags: u64) c_long {
    assert_bpf_program();
...
pub fn override_return(regs: *PtRegs, rc: u64) !void {
    assert_bpf_program();
...
pub fn get_stack(ctx: anytype, buf: []u8, flags: u64) usize {
    assert_bpf_program();
...
pub fn skb_load_bytes_relative(skb: anytype, offset: u32, to: []u8, start_header: u32) !void {
    assert_bpf_program();
...
pub fn rc_repeat(ctx: anytype) !void {
    assert_bpf_program();
...
///	This helper is only available is the kernel was compiled with the
///	**CONFIG_BPF_LIRC_MODE2** configuration option set to "**y**".
pub fn rc_keydown(ctx: anytype, protocol: u32, scancode: u64, toggle: u32) !void {
    assert_bpf_program();
...
pub const get_current_cgroup_id = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
...
pub fn get_local_storage(map: anytype, flags: u64) []u8 {
    assert_bpf_program();
...
///	the socket buffer.
pub fn sk_select_reuseport(reuse: *SkReuseportMd, map: *const MapDef, key: anytype, flags: u64) !void {
    assert_bpf_program();
...
///	This helper is only available is the kernel was compiled with the
///	**CONFIG_BPF_LIRC_MODE2** configuration option set to "**y**".
pub fn rc_pointer_rel(ctx: anytype, rel_x: i32, rel_y: i32) !void {
    assert_bpf_program();
...
///	sockets with reuseport option, the **struct bpf_sock** result is from
///	*reuse*\ **->socks**\ [] using the hash of the tuple.
pub fn skc_lookup_tcp(ctx: anytype, tuple: *SockTuple, tuple_size: u32, netns: u64, flags: u64) ?*Sock {
    assert_bpf_program();
...
/// otherwise.
pub fn tcp_check_syncookie(sk: *Sock, iph: []u8, th: *TcpHdr, th_len: u32) !void {
    assert_bpf_program();
...
///	range.
pub fn strtol(buf: []const u8, flags: u64, res: *c_long) !usize {
    assert_bpf_program();
...
///	unsupported base was provided.  **-ERANGE** if resulting value was out of
///	range.
pub fn strtoul(buf: []const u8, flags: u64, res: *c_ulong) !usize {
    assert_bpf_program();
...
pub fn send_signal(sig: u32) !void {
    assert_bpf_program();
...
pub fn tcp_get_syncookie(sk: *Sock, iph: []u8, th: *TcpHdr, th_len: u32) !u64 {
    assert_bpf_program();
...
pub fn tcp_send_ack(tp: anytype, rcv_nxt: u32) !void {
    assert_bpf_program();
...
///		**-EPERM** if no permission to send the *sig*.
///		**-EAGAIN** if bpf program can try again.
pub fn send_signal_thread(sig: u32) !void {
    assert_bpf_program();
...
pub const jiffies64 = if (in_userspace)
    @compileError("BPF helpers are not available in userspace")
else
    helpers.jiffies64;
...
pub fn get_ns_current_pid_tgid(dev: u64, ino: u64, nsdata: PidNsInfo, size: u32) !void {
    assert_bpf_program();
...
pub fn get_netns_cookie(ctx: anytype) u64 {
    assert_bpf_program();
...
pub fn get_current_ancestor_cgroup_id(ancestor_level: i32) ?u64 {
    assert_bpf_program();
...
pub fn skc_to_tcp6(sk: anytype) ?*Tcp6Sock {
    assert_bpf_program();
...
pub fn skc_to_tcp_sock(sk: anytype) ?*TcpSock {
    assert_bpf_program();
...
pub fn skc_to_tcp_timewait_sock(sk: anytype) ?*TcpTimewaitSock {
    assert_bpf_program();
...
pub fn skc_to_tcp_request_sock(sk: anytype) ?*TcpRequestSock {
    assert_bpf_program();
...
pub fn skc_to_udp6_sock(sk: anytype) ?*Udp6Sock {
    assert_bpf_program();
...
pub fn get_task_stack(task: *TaskStruct, buf: []u8, flags: u64) !usize {
    assert_bpf_program();

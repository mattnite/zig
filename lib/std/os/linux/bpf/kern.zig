...
pub const SkBuff = packed struct {
    ...
    pub const skb_under_cgroup: fn (skb: *SkBuff, map: *const CGroupArray, index: u32) !void = @compileError("TODO");
    ...
    pub const sk_redirect_map: fn (skb: *SkBuff, map: *const SockMap, key: u32, flags: u64) !void = @compileError("TODO");
    ...
    ///
    ///	Return **SK_PASS** on success, or **SK_DROP** on error.
    pub const sk_redirect_hash: fn (skb: *SkBuff, map: *const SockHash, key: anytype, flags: u64) !void = @compileError("TODO");
...
pub const SkMsgBuff = packed struct {
    ...
    /// to make the distinction (ingress path is selected if the flag is present,
    /// egress path otherwise). This is the only flag supported for now.
    pub const msg_redirect_map: fn (msg: *SkMsgBuff, map: *const SockMap, key: u32, flags: u64) !void = @compileError("TODO");
    ...
    ///
    ///	Return **SK_PASS** on success, or **SK_DROP** on error.
    pub const msg_redirect_hash: fn (msg: *SkMsgBuff, map: *const SockHash, key: anytype, flags: u64) !void = @compileError("TODO");
...
pub const ProgArrayMap = packed struct {
    ...
    /// *index* is superior to the number of entries in *prog_array_map*), or if
    /// the maximum number of tail calls has been reached for this chain of
...
pub const StackTrace = packed struct {
...
pub const CGroupArray = packed struct {
...
pub const Ringbuf = packed struct {
